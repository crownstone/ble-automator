...
if __name__ == '__main__':
	try:
		parser = optparse.OptionParser(usage="%prog [-vm] [-f <input file>] [-t] \n\nExample:\n\t%prog -f file.txt",
									version="0.1")
		...
		parser.add_option('-m', '--mesh',
				action="store_true",
				dest="mesh",
				help="Data was read via mesh characteristic."
				)
	...
	with open(options.data_file, 'r') as f:
		...
		for line in f.xreadlines():
			...
			if (options.mesh):
				...
				if (opcode == 255):
					sampleTimestamp = Conversion.uint8_array_to_uint32(msgData[0:4])
					samples = Conversion.uint8_array_to_uint16_array(msgData[4:4+43*2])
					sourceAddress = "local"
					if (sourceAddress in powerSamples):
						powerSamples[sourceAddress].extend(samples)
					else:
	...
	for addr in powerSamples:
		...
		# plt.plot(range(0,len(powerSamples[addr][0::2])), powerSamples[addr][0::2], "o-", label=addr)
		# plt.plot(range(0,len(powerSamples[addr][1::2])), powerSamples[addr][1::2], "o-", label=addr)
		plt.plot(range(0,len(powerSamples[addr])), powerSamples[addr], "o", label=addr)
		plt.legend()
